...
class DeepVerifyState(BaseModel):
    ...
    # NEW: targeted queries generated when fact-checking requires more research
    revision_queries: list[str] = Field(default_factory=list)
...
def merge_evidence(
    left: list[Evidence], right: list[Evidence]
) -> list[Evidence]:
...
class DeepVerifyGraphState(BaseModel):
    ...
    original_question: str
    research_plan: ResearchPlan | None = None
    evidence: Annotated[list[Evidence], merge_evidence] = Field(
        default_factory=list
    )
    draft: str | None = None
    claims: Annotated[list[str], merge_claims] = Field(
        default_factory=list
    )
    claim_checks: Annotated[
        list[ClaimCheck], merge_claim_checks
    ] = Field(default_factory=list)

    grounding_score: float | None = None
    revision_count: int = 0

    # NEW: targeted queries for the next research iteration
    revision_queries: list[str] = Field(default_factory=list)

    agent_events: Annotated[
        list[AgentEvent], merge_agent_events
    ] = Field(default_factory=list)

    errors: Annotated[
        list[str], merge_errors
    ] = Field(default_factory=list)
